...
public class UserController {
	...
	@ResponseBody
	public JsonResponse save(@RequestBody UserModel model) {
		try {
	...
	@ResponseBody
	public JsonResponse update(@RequestBody UserModel user) throws Exception {
		try {
	...
	public JsonResponse get(@PathVariable("id") String id) {
		try {
			UserModel model = service.get(id);
			return new JsonResponse(model);
		} catch (Exception e) {
			logger.error(e.getMessage());
		}
	...
	public JsonResponse getAll() {
		try {
			List<UserModel> users = service.findList();
	...
	public JSONObject getusersOfEasyui(@RequestParam(value = "account", required = false) String account, 
		...
		int total = 0;
		try {
			List<UserModel> users = service.getUsersByParams(account, nickname);
	...
	public JSONObject[] queryOfEasyui(@RequestParam(value = "id", required = false) String id) {
		...
		try {
			List<RoleModel> roles = roleService.findList();
			List<RoleModel> userRoles = service.getRoleListByUserId(id);
