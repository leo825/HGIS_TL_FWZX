...
//    @ResponseBody
//    public JsonResponse save(@RequestBody Route route) {
//        try {
...
//            if (count1 > -1) {
//                int count2 = initApplicationMethod.deleteServletRoute(route);
//                if (count2 > -1) {
...
//    @ResponseBody
//    public JsonResponse update(@RequestBody Route route) throws Exception {
//
//        Route oldRoute = null;
//        try {
...
//        try {
//            List<Route> projects = service.findList();
...
//                Project pModel = projectService.get((String) listAll.get(i).get("PROJECT_ID"));
...
//                    route.setServerName(String.valueOf(row.getCell(0)));
//                    route.setServerAddr(String.valueOf(row.getCell(1)));
//                    route.setDescription(String.valueOf(row.getCell(2)));
//                    route.setDataReturnType(String.valueOf(row.getCell(3)));
//                    if (service.insert(route) == -1) {
//                        logger.error("增加[ " + route.getServerName() + " ]接口失败了，因为在系统中已经存在了！！");
//                        faileCount++;
//                    } else {
//                        initApplicationMethod.addServletRoute(route);//接口在数据库中添加成功之后将其加入路由中
//                        logger.error("增加[ " + route.getServerName() + " ]成功！！");
//                    }
//                } catch (Exception e) {
